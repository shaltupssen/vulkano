...
impl Device {
    ...
    pub fn memory_win32_handle_properties(
        ...
        self.validate_memory_win32_handle_properties(handle_type, handle)?;
    ...
    fn validate_memory_win32_handle_properties(
        ...
        // Add Windows-specific handle validation using GetHandleInformation
        #[cfg(windows)]
        {
            let raw_handle = handle as winapi::shared::ntdef::HANDLE;
            let mut handle_info: winapi::shared::minwindef::DWORD = 0;

            if unsafe { winapi::um::handleapi::GetHandleInformation(raw_handle, &mut handle_info) }
                == 0
            {
                let error_code = unsafe { winapi::um::errhandlingapi::GetLastError() };
                return Err(Box::new(ValidationError {
                    problem: format!(
                        "handle is not a valid Windows handle. GetHandleInformation failed with error code: {}",
                        error_code
                    ).into(),
                    vuids: &["VUID-vkGetMemoryWin32HandlePropertiesKHR-handle-00665"],
                    ..Default::default()
                }));
            }
        }
